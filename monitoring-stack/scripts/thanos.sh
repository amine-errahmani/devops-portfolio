#!/bin/bash
action=$1
ENV=$2

function usage() {
    echo "usage :"
    echo " scripts/thanos.sh ACTION ENV"
    echo ""
    echo "example :"
    echo " scripts/thanos.sh upgrade dev"
    echo ""
    echo "available actions list :"
    echo "  diff-upgrade  -- executes a diff and shows what is to be applied in case of upgrade"
    echo "  upgrade  -- upgrades the helm chart"
    echo ""
    echo "available environments list :"
    echo "  dev"
    echo "  prod"
}

# check kubectl context
function contextCheck () {
    currentContext=$(kubectl config current-context | tr '[:upper:]' '[:lower:]')
    if [[ -z "$currentContext" ]]; then
        echo "can't find context, please ensure right context is set"
    elif [[ "$currentContext" == devops* ]] && [[ $ENV != "prod" ]]; then
        echo "wrong env/context combination: context is devops and env different form prod"
        echo "please set env to prod or change context"
        exit 1
    elif [[ "$currentContext" == devops* ]] && [[ $ENV != "dev" ]]; then
        echo "wrong env/context combination: context is devops and env different form dev"
        echo "please set env to dev or change context"
        exit 1
    elif [[ "$currentContext" != devops* ]] && [[ "$currentContext" != devops* ]]; then
        echo "current context is $currentContext"
        echo "wrong context: context should either be devops or devops"
        echo "please change context"
        exit 1
    fi
}

# https://github.com/bitnami/charts/tree/master/bitnami/thanos

## add bitnami repo to helm
    # helm repo add bitnami https://charts.bitnami.com/bitnami
    # helm repo update


if [[ $action == "upgrade" ]]; then
    contextCheck
    if [[ $ENV == 'dev' ]]; then
        ## install/upgrade Thanos chart
        # awk -v key="$shpthanosstakey" '{gsub("shpthanosstakey", key)}1' helm/thanos/values_prod.yaml > helm/thanos/values_prod.yaml.dec 
        helm upgrade thanos bitnami/thanos -f helm/thanos/values_dev.yaml --install --namespace monitoring-stack 
        # rm -f helm/thanos/values_prod.yaml.dec
    elif [[ $ENV == 'prod' ]]; then
        # awk -v key="$shpthanosstakey" '{gsub("shpthanosstakey", key)}1' helm/thanos/values_prod.yaml > helm/thanos/values_prod.yaml.dec
        helm upgrade thanos bitnami/thanos -f helm/thanos/values_prod.yaml --install --namespace monitoring-stack 
        # rm -f helm/thanos/values_prod.yaml.dec
    else
        echo "no valid env specified"
    fi
elif [[ $action == "diff-upgrade" ]]; then
    contextCheck
    if [[ $ENV == 'dev' ]]; then
        ## install/upgrade Thanos chart
        # awk -v key="$shpthanosstakey" '{gsub("shpthanosstakey", key)}1' helm/thanos/values_prod.yaml > helm/thanos/values_prod.yaml.dec 
        helm diff upgrade thanos bitnami/thanos -f helm/thanos/values_dev.yaml --install --namespace monitoring-stack --detailed-exitcode -C 10
        status=$?
        echo $status > helm/thanos/helm-diff-exitcode
        # rm -f helm/thanos/values_prod.yaml.dec
    elif [[ $ENV == 'prod' ]]; then
        # awk -v key="$shpthanosstakey" '{gsub("shpthanosstakey", key)}1' helm/thanos/values_prod.yaml > helm/thanos/values_prod.yaml.dec 
        helm diff upgrade thanos bitnami/thanos -f helm/thanos/values_prod.yaml --install --namespace monitoring-stack --detailed-exitcode -C 10
        status=$?
        echo $status > helm/thanos/helm-diff-exitcode
        # rm -f helm/thanos/values_prod.yaml.dec
    else
        echo "no valid env specified"
    fi
elif [[ $action == "help" ]]; then
    usage
else
    echo "no valid action specified"
    usage
fi