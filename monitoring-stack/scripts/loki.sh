...
function usage() {
    echo "usage :"
    echo "  scripts/loki.sh ACTION ENV"
    echo ""
    echo "example :"
    echo "  scripts/loki.sh upgrade dev"
    echo ""
...
if [[ $action == "upgrade" ]]; then
    if [[ $ENV == 'dev' ]]; then
        ...
        helm secrets upgrade --install loki grafana/loki --version $chart_version -f helm/loki/values_dev.yaml --namespace monitoring-stack --create-namespace
    elif [[ $ENV == 'prod' ]]; then
        helm secrets upgrade --install loki grafana/loki --version $chart_version -f helm/loki/values_prod.yaml --namespace monitoring-stack --create-namespace
    else
    ...
    if [[ $ENV == 'dev' ]]; then
        ## install/upgrade loki chart
        helm secrets diff upgrade --install loki grafana/loki --version $chart_version -f helm/loki/values_dev.yaml --namespace monitoring-stack --detailed-exitcode -C 10
        status=$?
        echo $status > helm/loki/helm-diff-exitcode
    elif [[ $ENV == 'prod' ]]; then
        helm secrets diff upgrade --install loki grafana/loki --version $chart_version -f helm/loki/values_prod.yaml --namespace monitoring-stack --detailed-exitcode -C 10
        status=$?
        echo $status > helm/loki/helm-diff-exitcode
    else
        echo "no valid env specified"
