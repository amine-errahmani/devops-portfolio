# locals {
#   solr_ss_name = "${var.environment_short}${trim(replace(trimspace(file(".version")), ".", ""), "v")}imcs"
# }

# resource "azurerm_linux_virtual_machine_scale_set" "app3_solr_vmscaleset" {
#   count                           = length(var.resource_group_locations)
#   name                            = "${local.solr_ss_name}${element(var.region_suffix, count.index)}"
...
#   sku                             = var.app3_VmSize
#   instances                       = element(var.solr_nb_instance, count.index)
...
#   computer_name_prefix = "${local.solr_ss_name}${element(var.region_suffix, count.index)}"
...
#   network_interface {
#     name    = "${local.solr_ss_name}${element(var.region_suffix, count.index)}-vnic"
#     primary = true
#     enable_accelerated_networking = true

#     ip_configuration {
#       name      = "${local.solr_ss_name}${element(var.region_suffix, count.index)}-IPConfiguration"
#       primary   = true
