# locals {
#   neo4j_ss_name = "${var.environment_short}${trim(replace(trimspace(file(".version")), ".", ""), "v")}imcn"
# }

# resource "azurerm_linux_virtual_machine_scale_set" "app3_neo4j_vmscaleset" {
#   count                           = length(var.resource_group_locations)
#   name                            = "${local.neo4j_ss_name}${element(var.region_suffix, count.index)}"
...
#   sku                             = var.app3_VmSize
#   instances                       = element(var.neo4j_nb_instance, count.index)
...
#   computer_name_prefix = "${local.neo4j_ss_name}${element(var.region_suffix, count.index)}"
...
#   network_interface {
#     name    = "${local.neo4j_ss_name}${element(var.region_suffix, count.index)}-vnic"
#     primary = true
#     enable_accelerated_networking = true

#     ip_configuration {
#       name      = "${local.neo4j_ss_name}${element(var.region_suffix, count.index)}-IPConfiguration"
#       primary   = true
...
#     Componenet  = var.tag_Component_app3
