locals {
  ss_name = "${var.environment_short}${trim(replace(trimspace(file(".version")), ".", ""), "v")}imim"
}

resource "azurerm_linux_virtual_machine_scale_set" "app2_vmscaleset" {
  ...
  source_image_id = element(var.app2_image_ids, count.index)
  ...
  tags = {
    ...
    Componenet  = var.tag_Component_app2
